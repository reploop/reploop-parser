...
public class JsonAstBuilder extends JsonBaseBaseVisitor<Node> {
	...
	private <D extends Comparable<D>, T, E> Optional<E> castIf(D val, T min, T max, Function<T, D> to,
			Function<D, E> cast) {
	...
	private Number toNumber(String val) {
		...
		Optional<ByteVal> ob = castIf(value, Byte.MIN_VALUE, Byte.MAX_VALUE, Byte::longValue,
				vb -> new ByteVal(vb.byteValue()));
		if (ob.isPresent()) {
			return ob.get();
		}
		Optional<ShortVal> os = castIf(value, Short.MIN_VALUE, Short.MAX_VALUE, Short::longValue,
				vs -> new ShortVal(vs.shortValue()));
		if (os.isPresent()) {
			return os.get();
		}
		Optional<IntVal> oi = castIf(value, Integer.MIN_VALUE, Integer.MAX_VALUE, Integer::longValue,
				vi -> new IntVal(vi.intValue()));
		if (oi.isPresent()) {
			return oi.get();
		}
	...
	private Number toFloat(String val) {
		...
		Optional<FloatVal> of = castIf(value, Float.MIN_VALUE, Float.MAX_VALUE, Float::doubleValue,
				vf -> new FloatVal(vf.floatValue()));
		if (of.isPresent()) {
			return of.get();
		}

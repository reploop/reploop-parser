 ...
 */
public class TypeSimplifyResolver extends AstVisitor<Node, MessageContext> {
    @Override
    ...
    public Field visitField(Field node, MessageContext context) {
        ...
        return new Field(node.getModifier(), node.getIndex(), node.getName(), ft, node.getValue(), node.getComments(), options);
    }

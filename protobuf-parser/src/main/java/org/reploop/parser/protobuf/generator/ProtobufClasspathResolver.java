...
public class ProtobufClasspathResolver extends AstVisitor<Node, Classpath<ProtoProgram>> {
	...
	/**
	 * The .proto file starts with a package declaration, which helps to prevent naming
	 * conflicts between different projects. In Java, the package name is used as the Java
	 * package unless you have explicitly specified a java_package, as we have here. Even
	 * if you do provide a java_package, you should still define a normal package as well
	 * to avoid name collisions in the Protocol Buffers name space as well as in non-Java
	 * languages.
	 */
	...
	public Field visitField(Field node, Classpath<ProtoProgram> context) {
		...
		return new Field(node.getModifier(), node.getIndex(), node.getName(), type, node.getValue(),
				node.getComments());
	}
				...
				if (typeName.size() == 1) {
					for (E entity : entities) {
						if (entity.getName().endsWith(typeName)) {
							...
							return Optional.of(new Field(field.getModifier(), field.getIndex(), field.getName(), ft,
									field.getValue(), field.getComments()));
						}
					}
				}
			}
	...
	public Message visitMessage(Message node, Classpath<ProtoProgram> context) {
		...
		return new Message(fullName, node.getComments(), fb.build(), ml, el, services, node.getOptions());
	}
	...
	public CommonPair visitCommonPair(CommonPair node, Classpath<ProtoProgram> context) {
		...
		if (JAVA_OUTER_CLASSNAME.equals(key)) {
			...
			String val;
			if (!isNullOrEmpty(outerClassname) && value instanceof StringValue
			...
			Boolean val;
			if (value instanceof BoolValue && null != (val = ((BoolValue) value).getValue()) && val) {
			...
			String val;
			if (value instanceof StringValue && isNotEmpty(val = ((StringValue) value).getValue())) {
	...
	public ProtoProgram visitProtobufProgram(ProtoProgram node, Classpath<ProtoProgram> context) {
		try {
			...
			ProtoProgram program = new ProtoProgram(node.getComments(), ob.build(), hb.build(), mb.build(), eb.build(),
					sb.build());

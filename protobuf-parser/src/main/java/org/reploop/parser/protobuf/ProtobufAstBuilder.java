...
import org.reploop.parser.protobuf.tree.ScalarType;
...
public class ProtobufAstBuilder extends Protobuf2BaseVisitor<Node> {
    ...
    public Field visitField(Protobuf2Parser.FieldContext ctx) {
        ...
        return new Field(modifier, getInt(ctx.INT()), name.getValue(), visitFieldType(ctx.fieldType()), value,
                comments(ctx.getStart()), obs.build());
    }

    @Override
    public ProtoProgram visitProgram(Protobuf2Parser.ProgramContext ctx) {
        return new ProtoProgram(comments(ctx.getStart()), visit(ctx.option(), Option.class),
                visit(ctx.header(), Header.class), visit(ctx.message(), Message.class),
                visit(ctx.enumeration(), Enumeration.class), visit(ctx.service(), Service.class));
    }

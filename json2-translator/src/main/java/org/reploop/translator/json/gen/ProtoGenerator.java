...
import org.reploop.parser.protobuf.tree.LongValue;
...
import static org.reploop.translator.json.support.Constants.EQ;
...
import static org.reploop.translator.json.support.Constants.PROTO;
...
public class ProtoGenerator extends AstVisitor<Node, BeanContext> {
    ...
    public Field visitField(Field node, BeanContext context) {
        ...
        Optional.ofNullable(node.getModifier()).ifPresent(modifier -> context.append(modifier.name()).whitespace());
        ...
        context.whitespace()
                ...
                .append(Constants.EQ)
                .whitespace()
                .append(String.valueOf(node.getIndex()));
    ...
    public Message visitMessage(Message node, BeanContext context) {
        ...
        return new Message(name, node.getComments(), fields, messages, enumerations, node.getServices(),
                node.getOptions());
    }

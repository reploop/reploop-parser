		...
		for (char c = it.first(); c != DONE; parent = node) {
			...
			if (ont.isPresent()) {
				...
				if (node.isEndOfAWord()) {
					startRangeMap.computeIfAbsent(startIndex, key -> new ArrayList<>())
						.add(new Range(startIndex, index));
				}
			}
			else {
				...
				node = root;
				startIndex = -1;
				// If this is the first mismatch after a match, we should re-search from
				// root.
				if (prevMatch) {
					prevMatch = false;
					continue;

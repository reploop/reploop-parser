...
public class RelativePathResolver extends AstVisitor<Node, MessageContext> {
    ...
    public Field visitField(Field node, MessageContext context) {
        ...
        return new Field(node.getModifier(), node.getIndex(), node.getName(), fieldType, node.getValue(), node.getComments(), node.getOptions());
    }
    ...
    public Message visitMessage(Message node, MessageContext context) {
        ...
        return new Message(node.getName(), node.getComments(), fields, messages, node.getEnumerations(), node.getServices(), options);
    }
}

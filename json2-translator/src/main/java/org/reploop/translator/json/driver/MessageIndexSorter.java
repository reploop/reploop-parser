...
public class MessageIndexSorter extends AstVisitor<Node, IndexContext> {
	...
	public Field visitField(Field node, IndexContext context) {
		return new Field(node.getModifier(), context.getAndIncrement(), node.getName(), node.getType(), node.getValue(),
	...
	public Message visitMessage(Message node, IndexContext context) {
		...
		List<Message> messages = visitIfPresent(node.getMessages(),
				message -> visitMessage(message, new IndexContext()));
		List<Enumeration> enumerations = visitIfPresent(node.getEnumerations(),
				enumeration -> visitEnumeration(enumeration, new IndexContext()));
		return new Message(node.getName(), node.getComments(), ordered, messages, enumerations, node.getServices(),
				node.getOptions());

...
import static org.reploop.translator.json.support.Constants.UNDERSCORE;
...
public class ClassHierarchy {
    ...
    private Optional<Parent> infer0(Map<QualifiedName, Message> messageMap) {
        ...
        if (op.isPresent()) {
            ...
            for (Message message : messages) {
                ...
                List<Field> sub = message.getFields()
                        .stream()
                        .filter(field -> !fields.contains(field))
                        .collect(toList());
                // Same as parent, we make this message as parent without modifying its
                // name.
                if (sub.isEmpty()) {
                    if (null == parent) {
                        parent = message;
                    ...
                    Message child = new Message(qn, message.getComments(), sub, message.getMessages(),
                            message.getEnumerations(), message.getServices(), message.getOptions());
            ...
            if (null == parent) {
                ...
                parent = new Message(name, new ArrayList<>(fields), Collections.emptyList(), Collections.emptyList(),
                        ops);
            } else {
                name = parent.getName();
                // Only 2 Fields
                if (subClasses.isEmpty() && fields.size() < 3) {
                    ...
                    parent = new Message(parentName, parent.getComments(), parent.getFields(), parent.getMessages(),
                            parent.getEnumerations(), parent.getServices(), parent.getOptions());
            ...
            if (!subClasses.isEmpty()) {
                ...
                for (Message sub : subClasses) {
                    ...
                    Message m = new Message(sub.getName(), sub.getComments(), sub.getFields(), sub.getMessages(),
                            sub.getEnumerations(), sub.getServices(), lb.build());
                    messageMap.put(m.getName(), m);
                }

                // If parent and subclasses are same actually, we eliminate some useless
                // entities.
    ...
    private QualifiedName parentName(Parent info) {
        ...
        // If parent class has less than 3 fields, try to generate a class name from
        // fields.
     ...
     */
    private void reduce(Message parent, List<Message> subClasses, Map<QualifiedName, Message> messageMap,
                        Map<QualifiedName, QualifiedName> sameMap) {
        // From bottom up
        List<Message> messages = subClasses.stream().sorted(messageComparator).collect(toList());
    ...
    private Optional<Message> reduce(Message parent, Message sub, Map<QualifiedName, Message> messageMap) {
        ...
        // A class has only one field and the field's type is same as it's parent, then we
        // consider this class can be parent.
        ...
        if (same) {
            ...
            List<Field> merge = ImmutableList.<Field>builder().add(field).addAll(parent.getFields()).build();
            List<Field> list = merge.stream().map(f -> dupTypeResolver.visitField(f, ctx)).distinct().collect(toList());
            Message msg = new Message(sub.getName(), sub.getComments(), list, sub.getMessages(), sub.getEnumerations(),
                    sub.getServices(), sub.getOptions());
            messageMap.put(msg.getName(), msg);

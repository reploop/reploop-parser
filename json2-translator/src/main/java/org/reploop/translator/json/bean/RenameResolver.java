...
import static org.reploop.translator.json.support.NameSupport.toLowerCamel;
...
public class RenameResolver extends AstVisitor<Node, MessageContext> {
    ...
    public Field visitField(Field node, MessageContext context) {
        ...
        return new Field(node.getModifier(), node.getIndex(), name, ft, node.getValue(), node.getComments(),
                obs.build());
    }
    ...
    public Enumeration visitEnumeration(Enumeration node, MessageContext context) {
        QualifiedName qn = rewriteQualifiedName(node.getName(), context);
        return new Enumeration(qn, node.getComments(),
                visitIfPresent(node.getFields(), f -> visitEnumField(f, context), EnumField.class));
    }

...
/**
 * All parser methods that used in grammar (p, prev, notLineTerminator, etc.) should start
...
public abstract class GoParserBase extends Parser {
	...
	/**
	 * Returns {@code true} iff on the current index of the parser's token stream a token
	 * exists on the {@code HIDDEN} channel which either is a line terminator, or is a
	 * multi line comment that contains a line terminator.
	 * @return {@code true} iff on the current index of the parser's token stream a token
	 * exists on the {@code HIDDEN} channel which either is a line terminator, or is a
	 * multi line comment that contains a line terminator.
	 */
	...
	/**
	 * Returns {@code true} if no line terminator exists between the specified token
	 * offset and the prior one on the {@code HIDDEN} channel.
	 * @return {@code true} if no line terminator exists between the specified token
	 * offset and the prior one on the {@code HIDDEN} channel.
	 */
	...
	/**
	 * Returns {@code true} if no line terminator exists after any encounterd parameters
	 * beyond the specified token offset and the next on the {@code HIDDEN} channel.
	 * @return {@code true} if no line terminator exists after any encounterd parameters
	 * beyond the specified token offset and the next on the {@code HIDDEN} channel.
	 */
